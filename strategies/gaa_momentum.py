...
def _fut():
    ...
    for name,sym in FUT.items():
        try:
            ...
            if s.empty: raise ValueError; out.append(s.rename(name).tz_localize(None))
        except Exception: warnings.warn(f"{name}: keine Daten")
...
def gaa_monthly_momentum() -> Tuple[str|None,str|None,str|None]:
    ...
    if hist.dropna().empty: return "GAA-Fehler",None,"Keine vollständigen Kurse."
    ...
    with open(HIST_FILE,"a") as f: f.write(f"{m_end:%F};{','.join(hold)}\n")
